...
pthread_mutex_t mutex_course_record[MAX_RECORDS];
int n_course_keys = 0;
pthread_mutex_t mutex_course_keys;

void intialize_course_record(char course_name[NAME_SIZE]){
  pthread_mutex_lock(&mutex_course_keys);
  strcpy(course_keys[n_course_keys],course_name); 
  pthread_mutex_init(&mutex_course_record[n_course_keys],NULL);
  n_course_keys++;
...
void intialize_course_resources() {
    ...
    while ((read(fd, &course, sizeof(struct Course)) > 0)) {
      intialize_course_record(course.course_name);
    }
    close(fd);
}
...
//     fcntl(fd, F_SETLKW, lock);
// }
void get_lock_all_course_record(){
  for(int i=0; i<n_course_keys; i++){
    pthread_mutex_lock(&mutex_course_record[i]);
  }
}
...
struct Array_faculty_course get_faculty_courses(char faculty_name[NAME_SIZE]) {
  ...
  for(int i = 0; i<n_rec; i++)
    pthread_mutex_unlock(&mutex_course_record[i]);
  close(fd);
...
int remove_course(struct Course course) {
  ...
  pthread_mutex_lock(&mutex_course_record[rec_no]);

  lseek(fd, rec_no* sizeof(struct Course), SEEK_SET);
  write(fd, &course, sizeof(struct Course));
  // release_course_lock(fd, &lock);
  pthread_mutex_unlock(&mutex_course_record[rec_no]);
  close(fd);
  return 0;
}
