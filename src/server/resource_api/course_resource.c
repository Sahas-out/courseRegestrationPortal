...
void get_lock_on_course_record(int fd, struct flock* lock, short lock_type, char course_name[NAME_SIZE]) {
    lock->l_type = lock_type;
    lock->l_whence = SEEK_SET;
    lock->l_start = (get_course_key(course_name)) * sizeof(struct Course);
...
void add_course(struct Course course) {
    struct flock lock;
    int fd;
...
void remove_course(struct Course course) {
    struct flock lock;
    int fd;
    ...
    course.active = 0;
    get_lock_on_course_record(fd, &lock, F_WRLCK, course.course_name);
    lseek(fd, get_course_key(course.course_name) * sizeof(struct Course), SEEK_SET);
    write(fd, &course, sizeof(struct Course));
